...
def local_get_stock_transaction(
        code,
        start,
        end,
        format='numpy',
...
def local_get_index_transaction(
        code,
        start,
        end,
        format='numpy',
...
def local_get_stock_list(collections=DATABASE.stock_list):
    '获取股票列表'
...
def local_get_etf_list(collections=DATABASE.etf_list):
    '获取ETF列表'
...
def local_get_index_list(collections=DATABASE.index_list):
    '获取指数列表'
    return pd.DataFrame([item for item in collections.find()]).drop(
        '_id',
        axis=1,
...
def local_get_stock_terminated(collections=DATABASE.stock_terminated):
    '获取股票基本信息 , 已经退市的股票列表'
...
def local_get_stock_basic_info_tushare(collections=DATABASE.stock_info_tushare):
    '''
    purpose:
        tushare 股票列表数据库
...
def local_get_stock_to_market_date(stock_code):
    '''
    根据tushare 的数据库查找上市的日期
    :param stock_code: '600001'
    :return: string 上市日期 eg： '2018-05-15'
    '''
    items = local_get_stock_basic_info_tushare()
    for row in items:
        if row['code'] == stock_code:
            return row['timeToMarket']


def local_get_stock_full(date, format='numpy', collections=DATABASE.stock_day):
    '获取全市场的某一日的数据'
    Date = str(date)[0:10]
...
def local_get_future_list(collections=DATABASE.future_list):
    '获取期货列表'
    return pd.DataFrame([item for item in collections.find()]).drop(
        '_id',
        axis=1,
...
def local_get_financial_report(code, report_date, ltype='EN', db=DATABASE):
    """获取专业财务报表
    Arguments:
        code {[type]} -- [description]
