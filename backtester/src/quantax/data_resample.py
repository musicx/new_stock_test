...
def tick_resample_1min(tick, type_='1min', if_drop=True):
    """
    tick 采样为 分钟数据
...
def tick_resample(tick, type_='1min'):
    """tick采样成任意级别分钟线
...
def ctptick_resample(tick, type_='1min'):
    """tick采样成任意级别分钟线
...
def min_resample(min_data, type_='5min'):
    """分钟线采样成大周期
...
def stockmin_resample(min_data, period=5):
    """
...
def min_to_day(min_data, type_='1D'):
    CONVERSION = {
        'code': 'first',
        'open': 'first',
        'high': 'max',
...
def futuremin_resample_to_day(
    ...
    """
    return min_data.assign(
        tradedate=pd.to_datetime(min_data.tradetime.apply(lambda x: x[0:10]))).reset_index().set_index(
        'tradedate').resample(type_). \
        apply({'code': 'first', 'open': 'first', 'high': 'max',
               'low': 'min', 'close': 'last', 'volume': 'sum'}).dropna()


def futuremin_resample_series(
        min_data,
        key='open',
        type_='5min',
        exchange_id=EXCHANGE_ID.SHFE
...
def day_resample(day_data, type_='w'):
    """日线降采样
...
def futureday_resample(day_data, type_='w'):
    """期货日线降采样
