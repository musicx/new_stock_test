...
def prepare_stock(code, year=2010):
...
def prepare_index(code, year=2010):
    ...
    formatted = data.data.reset_index().loc[:, ['date', 'open', 'high', 'low', 'close']]
    formatted.rename(columns={'date': 'datetime'}, inplace=True)
    return formatted.set_index('datetime')


class CashMarket(bt.Analyzer):
    def start(self):
...
class Benchmark(bt.Analyzer):
    ...
    def next(self):
        self.vals = self.strategy.datas[self.p.num].close[0]
        self.rets[self.strategy.datetime.datetime().strftime("%Y-%m-%d")] = self.vals
...
#######################################
class BasicTradeStats(bt.Analyzer):
    '''
    Summary:
    ...
    def create_analysis(self):
        ...
        o = self.rets = bt.utils.AutoOrderedDict()  # Return user object..
...
if __name__ == '__main__':
    ...
    elif (sys.argv[0] == 'quantstats'):
        import quantstats as qs

        stock_data = prepare_stock('300449', year=2018)
        index_data = prepare_index('000906', year=2018)

        stock = bt.feeds.PandasData(dataname=stock_data,
                                    fromdate=datetime.datetime(2019, 1, 1),
                                    todate=datetime.datetime(2020, 11, 30))
        index = bt.feeds.PandasData(dataname=index_data,
                                    fromdate=datetime.datetime(2019, 1, 1),
                                    todate=datetime.datetime(2020, 11, 30))

        cerebro.adddata(stock)
        cerebro.adddata(index)

        cerebro.addstrategy(TestStrategy)
        cerebro.addanalyzer(CashMarket, _name='cashmarket')
        cerebro.addanalyzer(Benchmark, num=1, _name='benchmark')
        cerebro.addsizer(bt.sizers.AllInSizer)
        results = cerebro.run()

        # ---- Format the values from results ----
        return_values = pd.DataFrame(results[0].analyzers.getbyname("cashmarket").get_analysis()).T
        return_values = return_values.iloc[:, 1]
        returns = qs.utils.to_returns(return_values)
        returns.index = pd.to_datetime(returns.index)
        # ----------------------------------------

        # ---- Format the benchmark ----
        bench_values = pd.DataFrame(results[0].analyzers.getbyname("benchmark").get_analysis(), index=('value',)).T
        bench_values.index = pd.to_datetime(bench_values.index)
        # -------------------------------------------

        qs.extend_pandas()
        qs.reports.html(returns, benchmark=bench_values, output="qs.html")
